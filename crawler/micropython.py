...
class GitHubSpider(scrapy.Spider):
    ...
    def start_requests(self):
        crawled = []
        for f in glob.glob("crawled/micropython/*"):
            try:
        ...
        try:
            with open("micropython_issues.json", "r") as f:
                issues = json.load(f)
        except:
            issues = []
            p = 1
            ...
            with open("micropython_issues.json", "w") as f:
                json.dump(issues, f)
    ...
    def parse(self, response):
        ...
        with open(f"crawled/micropython/{id}.json", "w") as f:
