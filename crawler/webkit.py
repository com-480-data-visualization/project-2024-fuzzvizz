...
class WebkitSpider(scrapy.Spider):
    name = __file__
    max_id = 1136160
    ...
    def start_requests(self):
        ...
        downloaded = set(
            map(lambda x: int(os.path.basename(x[0:-5])), glob.glob("crawled/webkit/*"))
        )

        urls = [
            f"https://bugs.webkit.org/show_bug.cgi?id={i}"
    ...
    def parse(self, response):
        ...
        for attachment in response.css("#attachment_table tr").xpath(".//b/.."):
            ...
            try:
                content = requests.get(url, timeout=1).content.decode(
        ...
        # Save data to file
        with open("crawled/webkit/" + str(id) + ".json", "w") as f:
            json.dump(bug, f)
