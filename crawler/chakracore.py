...
class GitHubSpider(scrapy.Spider):
    ...
    def start_requests(self):
        crawled = []
        for f in glob.glob("crawled/chakracore/*"):
            try:
        ...
        try:
            with open("chakracore_issues.json", "r") as f:
                issues = json.load(f)
        except:
            issues = []
            p = 1
            ...
            with open("chakracore_issues.json", "w") as f:
                json.dump(issues, f)
    ...
    def parse(self, response):
        ...
        with open(f"crawled/chakracore/{id}.json", "w") as f:
